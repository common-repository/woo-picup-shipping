...
final class PicupApi implements PicupApiInterface
{
     ...
     */
    private $apiPrefix = 'https://picupstaging-webapi.azurewebsites.net/v1/';
    ...
    private $endpointOrderStatus = '/integration/parcel-status/';
    private $endpointStageOrder = '/integration/stage-order';
    ...
    public function sendDeliveryBucket(DeliveryBucketRequest $deliveryBucket) : DeliveryBucketResponse
    {
        ...
        try {
            $response = $this->httpClient->post($this->apiPrefix . $this->endpointAddBucket, [
                'headers' => $headers,
                'json' => $deliveryBucket->jsonSerialize(),
            ]);
    ...
    public function sendStageOrder(StageOrderRequest $stageOrder)
    {
        ...
        try {
            $response = $this->httpClient->post($this->apiPrefix . $this->endpointStageOrder, [
                'headers' => $headers,
                'json' => $stageOrder,
            ]);
            ...
            if ($response = $e->getResponse()) {
                $msg = $response->getBody()->getContents();
                $this->checkResponseForErrors($msg, $stageOrder);
            }
            $errorMessage = 'DeliveryBucket Error: ' . $msg;

            throw new PicupRequestFailed($stageOrder, $errorMessage);
        }
    }
     ...
     * @throws PicupApiException  Should there be a general problem with the API
     */
    public function sendOrderStatusRequest(string $request)
    {
        $headers = ['api-key' => $this->apiKey];
        $endpoint = $this->apiPrefix . $this->endpointOrderStatus . $request;
        ...
        try {
            ...
            $body = json_decode($response->getBody()->getContents(), true);
                // OrderStatusResponseFactory::make($body)
    ...
    public function setLive() : void
    {
        ...
        $this->apiPrefix =  'https://picupprod-webapi.azurewebsites.net/v1/';
    }
    ...
    public function setTesting() : void
    {
        ...
        $this->apiPrefix = 'https://picupstaging-webapi.azurewebsites.net/v1/';
    }
